...
(() => {
  ...
  /* Elements that are a LINE BREAK as much as they are a wrapper.
     This set exists because of a bug that silently ate people's text. An
     unwrapped element used to lose its boundary along with its tag: the walk
     below spliced the children in and deleted the tag with nothing in its
     place. DIV is not in ALLOWED, and DIV is exactly what a contenteditable
     writes when you press Enter, so typing
         Monday⏎Ada to check the record
     was saved as "MondayAda to check the record" - the break deleted and the
     two words welded together. A blank line between paragraphs,
     <div><br></div>, collapsed the same way, which is why paragraph spacing
     "did not save".
     Unwrapping one of these now leaves a <br> where the boundary was, so the
     shape of what you typed survives even though the tag does not. It also
     fixes pasted content from Word and from web pages, which is mostly
     nested divs. */
  const BLOCKISH = new Set(["DIV", "SECTION", "ARTICLE", "HEADER", "FOOTER", "MAIN",
                            "ASIDE", "NAV", "BLOCKQUOTE", "PRE", "ADDRESS", "FIGURE",
                            "FIGCAPTION", "DL", "DT", "DD", "FORM", "FIELDSET",
                            "H1", "H2", "H3", "H4", "H5", "H6"]);

  /**
   * Is a break needed in front of this node, or is one already there?
   *
   * Whitespace-only text between block elements is not content - the parser
   * leaves it behind wherever the source HTML was indented - so it is skipped
   * when looking backwards. A node with nothing before it needs no boundary
   * either: a break at the very start of a field is a blank first line
   * nobody typed.
   */
  function needsBreakBefore(node) {
    ...
    if (prev.nodeType === 1 && prev.tagName.toUpperCase() === "BR") return false;
    return true;
  }
  ...
  function cleanHtml(dirty) {
    ...
    const walk = (node) => {
      ...
      for (const child of [...node.childNodes]) {
        ...
        walk(child);
        if (!ALLOWED.has(tag)) {
          // Unwrap: keep what the person wrote, lose the tag around it - and
          // for a block element, keep the LINE BREAK it stood for. Without
          // the <br> the boundary vanished with the tag and welded the two
          // lines into one word.
          const parent = child.parentNode;
          const block = BLOCKISH.has(tag);
          const empty = !child.firstChild;
          // Only where a break is not already standing.
          //
          // A blank line arrives as <div><br></div>, and that <br> IS the
          // boundary - adding another in front of it produced three breaks
          // where two were typed, so one blank line came back as two. Caught
          // by driving the real field, not by reading. So the boundary is
          // added only when what precedes this block is not already a break,
          // and the block's own leading break counts as one.
          if (block && !empty && needsBreakBefore(child)) {
            parent.insertBefore(doc.createElement("br"), child);
          }
          while (child.firstChild) parent.insertBefore(child.firstChild, child);
          // An empty block is a blank line with nothing inside it to stand in
          // for the boundary, so it leaves one behind.
          if (block && empty && needsBreakBefore(child)) {
            parent.insertBefore(doc.createElement("br"), child);
          }
          child.remove();
   ...
   *
   * The question this really answers is "would running esc() over it corrupt
   * it?", and the old fixed tag list got that wrong in a way people could
   * see. It tested for p, br, b, table and friends - so a saved value whose
   * only markup was an ENTITY, with no tag left in it at all, was judged
   * plain. It then went through esc() on the way back into the field, which
   * escaped the ampersand, and "Monday&nbsp;Ada" was displayed as the literal
   * characters "Monday&nbsp;Ada". That is the mystery text that appeared in
   * people's updates, and it appeared once per save because each round trip
   * escaped the previous round's ampersand again.
   *
   * So an entity counts as markup too. Anything genuinely plain - no angle
   * bracket, no entity - still takes the plain path and still gets its
   * newlines turned into breaks, which is what an older save needs.
   */
  const isHtml = (v) => {
    const s = String(v ?? "");
    // A tag: "<" followed by a letter or a closing slash. "a < b" is not one.
    return /<\/?[a-z][a-z0-9]*(\s|\/?>)/i.test(s) ||
           // A character reference: &nbsp; &amp; &#160; &#x27;
           /&(?:[a-z][a-z0-9]{1,9}|#[0-9]{1,7}|#x[0-9a-f]{1,6});/i.test(s);
  };
  ...
  /* ---------- the edit history behind a rich field ----------

     Why this is our own stack and not the browser's undo.

     execCommand("undo") works and queryCommandEnabled reports honestly, so
     the obvious version is two lines. It was measured in the real field
     before being rejected: typing three sentences in bursts 700ms apart and
     pressing undo once cleared the ENTIRE field, and redo brought all of it
     back. A discrete command like bold does get its own entry, but typing
     coalesces into a single transaction, so a button labelled Undo would
     have deleted everything somebody wrote in one click. That is a
     data-loss trap wearing the name of a safety feature.

     So the field keeps snapshots of its own, in the same shape as the undo
     history in store.js: a bounded stack, a redo stack that a fresh edit
     clears, and a depth read FROM the store rather than declared again here
     so the two can never drift apart. The behaviour is then identical in
     every browser instead of depending on each engine's idea of when one
     edit ends and the next begins.

     The stacks are keyed by field element and thrown away with the dialog. A
     half-typed update is not something to restore on the next open. */
  const richHist = new WeakMap();
  const RICH_QUIET = 500;        // ms of no typing that ends one step

  const histOf = (el) => {
    let h = richHist.get(el);
    if (!h) { h = { undo: [], redo: [], timer: 0, last: el.innerHTML }; richHist.set(el, h); }
    return h;
  };
  const richDepth = () =>
    (window.TrackerStore && window.TrackerStore.DEPTH) || 7;

  /**
   * Remember where this field is, so undo can come back to it.
   *
   * `now` skips the debounce, for a toolbar command: bold is one deliberate
   * act and belongs in the history immediately, where a run of keystrokes
   * should settle first or every character would be its own step.
   */
  function richSnapshot(el, { now = false } = {}) {
    const h = histOf(el);
    const take = () => {
      const cur = el.innerHTML;
      if (cur === h.last) return;               // nothing actually changed
      h.undo.push(h.last);
      while (h.undo.length > richDepth()) h.undo.shift();
      // A fresh edit ends the old future, exactly as store.js does.
      h.redo.length = 0;
      h.last = cur;
      paintRichHistory(el);
    };
    ...
    if (now) return take();
    h.timer = setTimeout(take, RICH_QUIET);
  }

  /** Step back one snapshot. Returns whether anything moved. */
  function richUndo(el) {
    const h = histOf(el);
    // A step still settling is committed first, or the last thing typed
    // would be skipped over and undo would feel one edit behind.
    clearTimeout(h.timer);
    if (el.innerHTML !== h.last) { h.undo.push(h.last); h.last = el.innerHTML; h.redo.length = 0; }
    if (!h.undo.length) return false;
    h.redo.push(h.last);
    while (h.redo.length > richDepth()) h.redo.shift();
    h.last = h.undo.pop();
    el.innerHTML = h.last;
    paintRichHistory(el);
    return true;
  ...
  /**
   * The two buttons, told what they can currently do.
   *
   * Deliberately the same shape as paintHistory() in app.js: the depth lives
   * in the accessible name rather than on the face of the button, and a
   * button with nothing to do is disabled rather than hidden so the toolbar
   * does not reflow as you type.
   */
  function paintRichHistory(el) {
    const doc = el.ownerDocument;
    const bar = doc.querySelector(`[data-richbar="${cssEsc(el.id)}"]`);
    if (!bar) return;
    const h = histOf(el);
    const set = (sel, n, word) => {
      const b = bar.querySelector(sel);
      if (!b) return;
      const label = n ? `${word} (${n} of ${richDepth()})` : `Nothing to ${word.toLowerCase()}`;
      b.disabled = !n;
      b.title = label;
      b.setAttribute("aria-label", label);
    };
    set(`[data-richundo]`, h.undo.length, "Undo");
    set(`[data-richredo]`, h.redo.length, "Redo");
  }
  ...
  function richToolbar(id) {
    ...
    // Undo and redo lead the bar, in the same two glyphs the sidebar pair
    // uses (ICONS.undo / ICONS.redo), so they read as the same control rather
    // than as two unrelated buttons. tabindex -1 like every other rich
    // button: clicking one must never take focus off the text.
    const hist = (which, word) =>
      `<button type="button" class="richbtn" data-rich${which}="${id}" data-for="${id}"
               title="Nothing to ${word}" aria-label="Nothing to ${word}"
               tabindex="-1" disabled>
         <svg viewBox="0 0 24 24" aria-hidden="true">${ICONS[which] || ""}</svg>
       </button>`;
    return `<div class="richbar" data-richbar="${id}">
        ${hist("undo", "undo")}${hist("redo", "redo")}
   ...
   */
  /**
   * Wire the rich fields in a dialog, and hand back a way to unwire them.
   *
   * The teardown is the whole point of the return value. ensureHost() gives
   * every dialog the SAME host element, and this function used to add its
   * listeners to that host and to document and never remove them. They
   * accumulated one set per dialog opened, and the effect was silent and
   * bizarre: after five dialogs, ONE click on the bold button ran
   * execCommand five times, so a toggle applied an odd or an even number of
   * times depending on how many dialogs you had opened that session. Bold
   * appearing to do nothing is what that looks like from the outside.
   *
   * Measured on the unmodified build before this was changed: five dialogs
   * opened, then one click on the list button, and execCommand was called
   * five times. formDialog now calls the teardown in close(), beside the
   * attachment URLs it already revokes there, so nothing is left behind.
   */
  function wireRich(box) {
    const boxes = [...box.querySelectorAll("[data-rich]")];
    const bound = [];
    /** addEventListener that remembers, so off() can undo all of it. */
    ...
    on(box, "keyup", paintState);
    on(box, "mouseup", paintState);
    on(document, "selectionchange", paintState);

    /* Every listener below is bound to the FIELD, never to the dialog host.
       That is not a style choice, it is a bug this cost.

       ensureHost() returns ONE element reused by every dialog, and
       formDialog's close() removes only its own two listeners. A handler
       added to the host therefore survives the dialog that added it and
       accumulates: by the sixth dialog of a session there were six copies,
       so one click on undo ran it six times and emptied the field in a
       single press. Caught by driving six dialogs in a row - a single
       dialog looks perfect, which is why the first test of this passed.

       The fields and their toolbars are rebuilt for every dialog, so a
       listener on one of those dies with it and cannot pile up. */
    for (const el of boxes) {
      // The starting state is the baseline, so the buttons begin correctly
      // disabled rather than offering to undo a field nobody has touched.
      histOf(el).last = el.innerHTML;
      paintRichHistory(el);
      ...
      /* Ctrl+Z and Ctrl+Shift+Z drive the SAME history as the buttons.
         ...
         run ours, and the field jumps between two different pasts - a worse
         fault than the one this feature fixes. */
      on(el, "keydown", (e) => {
        if (!(e.metaKey || e.ctrlKey)) return;
        const k = String(e.key || "").toLowerCase();
        if (k !== "z" && k !== "y") return;
        e.preventDefault();
        // Ctrl+Y is redo on Windows; Ctrl+Shift+Z is redo everywhere else.
        if (k === "y" || e.shiftKey) richRedo(el); else richUndo(el);
        paintState();
      });

      /* Undo and redo from the toolbar, on the bar itself. Deliberately NOT
         routed through the data-cmd handler below: that calls execCommand,
         which is the browser's own history, and the two stacks would
         disagree about what the past is. */
      const bar = box.querySelector(`[data-richbar="${cssEsc(el.id)}"]`);
      if (!bar) continue;
      on(bar, "click", (e) => {
        const u = e.target.closest("[data-richundo]");
        const r = u ? null : e.target.closest("[data-richredo]");
        if (!u && !r) return;
        e.preventDefault();
        if (u) richUndo(el); else richRedo(el);
        el.focus();
        paintState();
      });
    }
    ...
    on(box, "click", (e) => {
      ...
      if (cmd) {
        ...
        if (!el) return;
        el.focus();
        // Snapshotted before the command, immediately rather than on the
        // debounce: bold is one deliberate act and undo should step over
        // exactly it, not over it plus whatever was typed just before.
        richSnapshot(el, { now: true });
    ...
    // Handed to formDialog, which calls it in close(). See the note above:
    // without this, every listener here survives its dialog and piles up.
    return { off };
  }
  ...
  function formDialog({ title, intro, fields, submitLabel = "Save", choices = null,
    ...
    return new Promise((resolve) => {
      const close = (value) => {
        atts.revoke();
        // Every listener the rich fields registered goes with the dialog.
        // Left behind they accumulate on the shared host, and one click on a
        // toolbar button then runs its command once per dialog ever opened.
        rich.off();
        box.hidden = true;
        box.innerHTML = "";
  ...
  window.TrackerUI = { saveBlob, openBlob, formDialog, richUndo, richRedo, richSnapshot, confirmDialog, htmlDialog, cleanHtml, htmlText, isHtml, ATT_MAX,
